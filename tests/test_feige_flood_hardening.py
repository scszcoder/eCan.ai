...
* ``_evaluate_js`` gains a ``read_only`` param; read-only timeouts skip
  ``mark_live_chat_cdp_unhealthy`` and ``_record_cdp_evaluate_recovery_signal``.
* ``_evaluate_live_chat_js`` helper resolves the Feige target id and runs with
...
_SITE = Path("agent/ec_skills/browser_use_extension/hooks/external/feige_chat/site_tools.py")
...
class TestEvaluateJsReadOnly(unittest.TestCase):
    ...
    def test_read_only_timeout_skips_unhealthy_and_recovery(self):
        ...
        # The read-only branch must come before mark_live_chat_cdp_unhealthy /
        ...
        unhealthy_idx = handler.index("mark_live_chat_cdp_unhealthy(")
...
class TestEvaluateFeigeJsHelper(unittest.TestCase):
    ...
    def setUpClass(cls):
        cls.svc = _SVC.read_text(encoding="utf-8")
        cls.site = _SITE.read_text(encoding="utf-8")

    def test_helper_defined(self):
        self.assertIn("async def _evaluate_live_chat_js(", self.svc)

    def test_helper_resolves_target_and_uses_focus_false(self):
        i = self.svc.index("async def _evaluate_live_chat_js(")
        j = self.svc.index("\ndef ", i + 1)
        body = self.svc[i:j]
        self.assertIn("_resolve_live_chat_tab_target_id_bounded(browser_session)", body)
    ...
    def test_feige_tools_use_helper(self):
        ...
        # (tool bodies live in the bundle's site_tools.py since round 2)
        for fn in ("feige_open_session", "feige_list_sessions", "feige_get_chat_thread"):
            i = self.site.index(f"async def {fn}(")
            j = self.site.index("\nasync def ", i + 1) if "\nasync def " in self.site[i + 1:] else len(self.site)
            # find next top-level def of any kind
            k = self.site.index("\n@custom_controller.action", i + 1) if "\n@custom_controller.action" in self.site[i + 1:] else j
            body = self.site[i:min(j, k) if k > i else j]
            self.assertIn("_evaluate_live_chat_js(", body, f"{fn} should call _evaluate_live_chat_js")
...
class TestHardenedConstants(unittest.TestCase):
    def test_recovery_thresholds_bumped(self):
        ...
        self.assertIn('"ECAN_LIVE_CHAT_CDP_EVALUATE_RECOVERY_THRESHOLD") or "3"', svc)

    def test_cdp_health_cooldown_reduced(self):
        svc = _SVC.read_text(encoding="utf-8")
        self.assertIn('"ECAN_LIVE_CHAT_CDP_HEALTH_COOLDOWN_S") or "4.0"', svc)
        # Old 8.0 default must be gone.
        self.assertNotIn('"ECAN_LIVE_CHAT_CDP_HEALTH_COOLDOWN_S") or "8.0"', svc)

    def test_direct_circuit_relaxed(self):
        runner = _RUNNER.read_text(encoding="utf-8")
        self.assertIn('"DIRECT_LIVE_CHAT_CDP_TIMEOUT_CIRCUIT_THRESHOLD") or "2"', runner)
        self.assertIn('"DIRECT_LIVE_CHAT_CDP_TIMEOUT_CIRCUIT_COOLDOWN_S") or "6.0"', runner)
        self.assertNotIn('"DIRECT_LIVE_CHAT_CDP_TIMEOUT_CIRCUIT_COOLDOWN_S") or "20.0"', runner)
...
class TestRuntimeValuesLoaded(unittest.TestCase):
    ...
    def test_extension_tools_constants(self):
        ...
        self.assertEqual(ets._LIVE_CHAT_CDP_EVALUATE_RECOVERY_THRESHOLD, 3)
        self.assertEqual(ets._LIVE_CHAT_CDP_HEALTH_COOLDOWN_S, 4.0)
