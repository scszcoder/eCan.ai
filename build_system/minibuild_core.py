...
class MiniSpecBuilder:
    ...
    def _write_spec(self, mode: str) -> Path:
        ...
        app_name = app.get("name", "eCan")  # Use consistent app name
        app_version = app.get("version", "1.0.0")  # Get version from config
        ...
        runtime_tmpdir = None
        if runtime_tmpdir_cfg:
            import sys
            if sys.platform.startswith('win'):
                runtime_tmpdir = runtime_tmpdir_cfg.get("windows")
            elif sys.platform == 'darwin':
                runtime_tmpdir = runtime_tmpdir_cfg.get("macos")
            else:
                runtime_tmpdir = runtime_tmpdir_cfg.get("linux")
        ...
        spec_lines.append('"""')
        spec_lines.append(f'Generated PyInstaller spec for {app_name} - {mode} mode')
        spec_lines.append('Auto-generated by eCan build system')
        spec_lines.append('"""')
        spec_lines.append("")
        spec_lines.append("import sys")
        spec_lines.append("from pathlib import Path")
        spec_lines.append("")
        spec_lines.append("# Project root detection")
        spec_lines.append("import os as _os, sys as _sys")
        spec_lines.append(f"project_root = Path(r'{str(self.project_root)}')")
        spec_lines.append("print(f'[SPEC] Project root: {project_root}')")
        spec_lines.append("")
        ...
        # Dynamic icon path detection
        spec_lines.append("# Dynamic icon path detection")
        spec_lines.append("icon_path = None")
        spec_lines.append("if _sys.platform.startswith('win'):")
        spec_lines.append("    icon_candidates = [")
        spec_lines.append(f"        project_root / '{icon_win}',")
        spec_lines.append("        project_root / 'resource' / 'icon.ico',")
        spec_lines.append("        project_root / 'resource' / 'eCan.ico'")
        spec_lines.append("    ]")
        spec_lines.append("elif _sys.platform == 'darwin':")
        spec_lines.append("    icon_candidates = [")
        spec_lines.append(f"        project_root / '{icon_mac}',")
        spec_lines.append("        project_root / 'resource' / 'icon.icns',")
        spec_lines.append("        project_root / 'resource' / 'eCan.icns'")
        spec_lines.append("    ]")
        spec_lines.append("else:")
        spec_lines.append("    icon_candidates = []")
        spec_lines.append("")
        spec_lines.append("for candidate in icon_candidates:")
        spec_lines.append("    if candidate.exists():")
        spec_lines.append("        icon_path = str(candidate)")
        spec_lines.append("        print(f'[SPEC] Using icon: {icon_path}')")
        spec_lines.append("        break")
        spec_lines.append("")
        spec_lines.append("if not icon_path:")
        spec_lines.append("    print('[SPEC] Warning: No icon file found')")
        spec_lines.append("")
        ...
        # Add custom hooks path for unified symlink management (portable)
        spec_lines.append("    hookspath=[str(project_root / 'build_system' / 'pyinstaller_hooks')],  # Custom hooks for symlink management")
        ...
        spec_lines.append("print(f'[SPEC] Final data files count: {len(a.datas)}')")
        spec_lines.append("print(f'[SPEC] Final binaries count: {len(a.binaries)}')")
        spec_lines.append("print(f'[SPEC] Final hidden imports count: {len(hiddenimports)}')")
        spec_lines.append("")
        ...
        if onefile:
            ...
            spec_lines.append("    strip=False if _sys.platform.startswith('win') else True,")
            spec_lines.append("    upx=False,")
            spec_lines.append("    upx_exclude=[],")
            spec_lines.append(f"    runtime_tmpdir={repr(runtime_tmpdir)},")
            spec_lines.append(f"    console={repr(console)},")
            spec_lines.append("    icon=icon_path,")
            spec_lines.append(")")
        else:
            # onedir EXE + COLLECT
            spec_lines.append("exe = EXE(")
            ...
            spec_lines.append("        icon=icon_path,")
            spec_lines.append("        bundle_identifier='com.ecan.app',")
            spec_lines.append("        info_plist={")
            spec_lines.append("            'CFBundleName': '" + app_name + "',")
            spec_lines.append("            'CFBundleDisplayName': '" + app_name + "',")
            spec_lines.append("            'CFBundleVersion': '" + app_version + "',")
            spec_lines.append("            'CFBundleShortVersionString': '" + app_version + "',")
            spec_lines.append("            'NSHighResolutionCapable': True,")
            spec_lines.append("            'NSRequiresAquaSystemAppearance': False,")
            spec_lines.append("            'LSMinimumSystemVersion': '10.14.0',")
            spec_lines.append("            'NSAppTransportSecurity': {'NSAllowsArbitraryLoads': True},")
            spec_lines.append("            'NSMicrophoneUsageDescription': '" + app_name + " needs microphone access for voice features',")
            spec_lines.append("            'NSCameraUsageDescription': '" + app_name + " needs camera access for visual features',")
            spec_lines.append("            'NSNetworkVolumesUsageDescription': '" + app_name + " needs network access for automation',")
            spec_lines.append("            'NSAppleEventsUsageDescription': '" + app_name + " needs AppleEvents access for system automation',")
            spec_lines.append("            'NSSystemAdministrationUsageDescription': '" + app_name + " needs admin access for system automation',")
            spec_lines.append("        },")
            spec_lines.append("    )")
