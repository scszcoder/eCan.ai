...
class BuildValidator:
    ...
    def _validate_windows_artifacts(self, version: str, arch: str) -> List[Dict[str, Any]]:
        ...
        # Check standardized executable (supports multiple naming formats)
        std_exe_patterns = [
            f"dist/eCan-{version}-windows-{arch}.exe",  # Format generated by build script
            f"dist/eCan-Setup-windows-{arch}-v{version}.exe",  # Installer renamed format
        ]
        ...
        # Check installer (optional) - may be the same file as standardized executable
        installer_patterns = [
            f"dist/eCan-Setup.exe",  # Original installer name
            f"dist/eCan-Setup-windows-{arch}-v{version}.exe",  # Renamed installer
        ]
        ...
        if not installer_found:
            # Check if installer was renamed to standardized executable
            if std_exe_found:
                checks.append({
                    "name": "installer",
                    "status": "pass",
