...
def _canonical_run_body(body: str) -> str:
    ...
    # Per-pipeline requirements files are legitimate divergence — the
    # symmetry check already normalizes them. We have two symmetric pairs:
    #   * build jobs: `requirements-cn.txt` ↔ `requirements-intl.txt` (full runtime)
    #   * storage jobs (upload / appcast / latest-json):
    #     `build_system/scripts/requirements-cos.txt` ↔ `build_system/scripts/requirements.txt`
    #     (narrow set: cos-python-sdk-v5 vs boto3, both + pyyaml + packaging)
    # Strip all four so the parity check reports genuine differences only.
    out = re.sub(r'requirements-(?:cn|intl)\.txt',
                 'requirements-<APP>.txt', out)
    out = re.sub(r'build_system/scripts/requirements(?:-cos)?\.txt',
                 'build_system/scripts/requirements-<APP>.txt', out)
