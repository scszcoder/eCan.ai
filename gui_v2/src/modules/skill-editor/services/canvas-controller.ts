...
class CanvasControllerService {
  ...
  async loadFlowgram(flowgramData: {
    ...
    try {
      ...
      // Convert nodes to the native editor format.
      // The backend model_dump produces nodes with:
      //   meta.position, data.title, data.inputsValues, blocks, internal_edges/edges
      // The editor (initial-data / loadBundle) expects:
      //   meta.position, data (with title, inputsValues, etc.), top-level blocks & edges
      const convertedNodes = flowgramData.nodes.map((node: any) => {
        // Position: prefer meta.position (native), fall back to top-level position
        const pos =
          node.meta?.position ??
          node.position ??
          { x: 100, y: 100 };

        // Data: prefer node.data (native, has title/inputsValues), fall back to assembled
        const dataObj = (node.data && typeof node.data === 'object' && Object.keys(node.data).length > 0)
          ? { ...node.data }
          : {
        ...
        // Ensure title exists
        if (!dataObj.title) {
          dataObj.title = node.label || node.title || node.id;
        }
        ...
        // Loop nodes: blocks and internal edges at TOP LEVEL (not inside data)
        const blocks = node.blocks;
        if (Array.isArray(blocks) && blocks.length > 0) {
          result.blocks = blocks;
        }
        // Internal edges: try internal_edges first, then edges (on loop nodes only)
        const internalEdges = node.internal_edges ?? (node.type === 'loop' ? node.edges : undefined);
        if (Array.isArray(internalEdges) && internalEdges.length > 0) {
          result.edges = internalEdges;
        }

        return result;
      });
      
      // Convert edges to the native editor format (sourceNodeID/targetNodeID).
      // Accept multiple legacy key names from the backend.
      const convertedEdges = flowgramData.edges.map((edge: any) => {
        const source =
          edge.sourceNodeID ??
          edge.source ??
          edge.sourceNodeId ??
        ...
        const edgeObj: Record<string, any> = {
          sourceNodeID: source,
          targetNodeID: target,
        };
        if (sourcePortID) edgeObj.sourcePortID = sourcePortID;
        if (targetPortID) edgeObj.targetPortID = targetPortID;
