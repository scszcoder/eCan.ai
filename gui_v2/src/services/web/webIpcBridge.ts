...
//  * 
//  * Please use IPCAPI from '@/services/ipc/api' instead.
//  * IPCAPI.executeRequest() automatically handles routing between GraphQL and IPC.
//  * 
//  * Migration guide:
//  * - All API calls should go through IPCAPI.getInstance().executeRequest()
//  * - IPCAPI has built-in support for shouldPreferWebBridgeForMethod()
//  * - No need for manual platform detection or web bridge fallback
//  * 
//  * See: gui_v2/src/services/ipc/api.ts for the unified API interface
//  */
...
//       }
//       // Direct GraphQL call - bypass webApi layer
//       const READ_SKILL_FILE_QUERY = `query ReadSkillFile($filePath: String!) { readSkillFile(filePath: $filePath) { content filePath fileName fileSize skillName } }`;
//       const data = await appSyncRequest<{ readSkillFile: any[] }>(READ_SKILL_FILE_QUERY, { filePath }, undefined, 'read_skill_file');
//       const content = data.readSkillFile;
...
//       }
//       // Direct GraphQL call - bypass webApi layer
//       const OPEN_SKILL_FILE_QUERY = `query OpenSkillFile($filePath: String!, $skillName: String) { openSkillFile(filePath: $filePath, skillName: $skillName) { content filePath fileName fileSize skillName } }`;
//       const data = await appSyncRequest<{ openSkillFile: any }>(OPEN_SKILL_FILE_QUERY, { filePath, skillName }, undefined, 'open_skill_file');
//       const content = data.openSkillFile;
//       if (!content) {
//         return { success: false, error: { code: 'NOT_FOUND', message: 'Skill file not found.' } } as any;
//       }
...
//       }
//       // Direct GraphQL call - bypass webApi layer
//       const WRITE_SKILL_FILE_MUTATION = `mutation WriteSkillFile($input: [SkillFileInput!]!) { writeSkillFile(input: $input) { filePath fileName fileSize skillName updatedAt } }`;
//       const data = await appSyncRequest<{ writeSkillFile: any[] }>(WRITE_SKILL_FILE_MUTATION, { input: [{ filePath, content }] }, undefined, 'write_skill_file');
