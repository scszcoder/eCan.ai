...
async function main() {
  ...
  try {
    ...
    const orgs = await prisma.org.createMany({
      data: [
        {
          id: 'org-root',
          name: 'eCan.ai 总部',
    ...
    console.log(`  ✓ 创建了 ${orgs.count} 个组织\n`);
    ...
    await prisma.setting.createMany({
      data: [
        {
          key: 'system.app_name',
          value: { value: 'eCan.ai', description: '应用名称' },
        },
        {
          key: 'system.version',
          value: { value: '1.0.0', description: '系统版本' },
        },
        {
          key: 'system.timezone',
          value: { value: 'Asia/Shanghai', description: '系统时区' },
        },
        {
          key: 'agent.default_status',
          value: { value: 'active', description: 'Agent 默认状态' },
        },
        {
          key: 'task.default_priority',
          value: { value: 'normal', description: '任务默认优先级' },
        },
      ],
    });
    ...
    console.log(`  ✓ 创建了 Agent: ${agent.name}\n`);
    ...
    console.log(`  ✓ 创建了 Skill: ${skill.name}\n`);

    // ============ 5. 创建 Agent-Skill 关联 ============
    console.log('🔗 创建关联关系...');
    
    await prisma.agentSkillRel.create({
      data: {
        agentId: 'agent-demo',
        skillId: 'skill-demo',
        proficiencyLevel: 80,
    ...
    console.log('  ✓ Agent-Skill 关联创建完成\n');
