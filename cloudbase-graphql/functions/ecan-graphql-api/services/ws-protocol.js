 ...
 */
function createConnectionState({ connectionId, send, log = () => {} }) {
  const state = {
    connectionId,
...
function handleClientMessage(state, raw, opts = {}) {
  ...
  switch (frame.type) {
    ...
    case 'start': {
      ...
      let iterator;
      try { iterator = bus.subscribe(r.topic, r.target, ctx); }
      catch (e) {
        sendFrame(state, { type: 'error', id, payload: [{ message: `subscribe failed: ${e.message}` }] });
        return { close: false };
      }

      const sub = { topic: r.topic, target: r.target, ctx, iterator, pump: null, id };
      state.subscriptions.set(id, sub);
      sendFrame(state, { type: 'start_ack', id });

      // Spawn the per-subscription pump. We never await it inside handleClientMessage;
      // errors are caught and emitted as `error` frames so the client can react.
