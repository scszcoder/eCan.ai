...
async function main() {
  ...
  if (r.statusCode === 200 && r.body.includes('ecan-graphql-ws')) ok('/healthz returns 200');
  ...
  if (pr.statusCode === 200 && JSON.parse(pr.body).success) ok('/publish correct secret → 200');
  ...
  if (pr.statusCode === 400) ok('/publish unknown topic → 400');
