...
async function sendA2AMessage(prisma, identity, input) {
  ...
  // subscribers. Publish to in-process event-bus; cross-instance delivery
  // via ws-bridge-push.js (services/ws-bridge-push.js).
  // (The AWS WS path has the same gap; see cn-publishers.js header for rationale.)
...
async function publishSkillEditorEvent(prisma, identity, input) {
  ...
  // cross-instance delivery via ws-bridge-push.js.
  const bus = require('../event-bus');
