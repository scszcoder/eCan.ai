...
class MainWindow:
    def __init__(self, auth_manager: AuthManager, mainloop, ip,
        ...
        logger.info("[MainWindow] ðŸš€ Starting optimized MainWindow initialization...")
    ...
    async def _update_vehicle_metrics_async(self, vehicle):
        ...
        try:
            logger.debug(f"[MainWindow] ðŸ“Š Starting async metrics update for vehicle: {vehicle.getName()}")
    ...
    def update_all_llms(self, reason="unknown"):
        ...
        """
        try:
            logger.info(f"[MainWindow] ðŸ”„ Updating all LLMs - Reason: {reason}")
            ...
            logger.error(f"[MainWindow] âŒ Error updating LLMs: {e}")
            import traceback
        ...
        try:
            ...
            def on_proxy_change(proxies):
                ...
                if proxies:
                    ...
                    logger.info(f"[MainWindow] ðŸŒ Proxy enabled - {proxy_info}")
                    reason = f"Proxy enabled - {proxy_info}"
                else:
                    logger.info("[MainWindow] ðŸŒ Proxy disabled - using direct connection")
    ...
    async def _async_background_initialization(self):
        ...
        """
        try:
            logger.info("[MainWindow] ðŸ”„ Starting background initialization phase...")

            # Phase 2A: Database and critical services (parallel where possible)
            logger.info("[MainWindow] ðŸ“Š Initializing database and critical services...")
            ...
            logger.info("[MainWindow] ðŸš— Checking vehicles after database services ready...")
            await asyncio.get_event_loop().run_in_executor(
                None, self._check_vehicles_with_database
            )

            # Phase 2B: Data loading (must be after database services are ready)
            logger.info("[MainWindow] ðŸ“‚ Loading local data...")
            await asyncio.get_event_loop().run_in_executor(
            ...
            logger.info("[MainWindow] ðŸ¤– Initializing servers and agents...")
            ...
            logger.error(f"[MainWindow] âŒ Background initialization failed: {e}")
            import traceback
    ...
    def _test_push_demo_ad(self):
        ...
        try:
            ...
            def delayed_push():
                ...
                push_ad_to_frontend(
                    banner_text='ðŸŽ‰ Welcome! Special Offer: 50% off Premium Plans - Limited Time Only! Click for details.',
                    popup_html='''
                        <div style="text-align: center; padding: 20px;">
                            <h2 style="color: #f8fafc; margin-bottom: 12px;">ðŸŽ‰ Special Offer!</h2>
    ...
    async def _finalize_async_initialization(self):
        ...
        logger.info("[MainWindow] ðŸ Finalizing async initialization...")
        ...
        if hasattr(self, 'vehicles') and self.vehicles and hasattr(self, 'vehicle_service') and self.vehicle_service:
            logger.info("[MainWindow] ðŸš— Saving vehicles to database (deferred from sync phase)...")
        ...
        if getattr(self, '_should_start_cloud_sync', False):
            logger.info("[MainWindow] ðŸŒ Starting deferred cloud data sync...")
            try:
                loop = asyncio.get_running_loop()
        ...
        try:
            ...
            logger.debug("[MainWindow] ðŸ“‹ Scheduled LLM provider onboarding check (non-blocking)")
        except RuntimeError as e:
    ...
    #             logger.debug(f"[MainWindow] ðŸ“‚ Source my_skills directory not found: {source_dir}")
    #             return
            
    #         # Create target directory if it doesn't exist
    #         if not os.path.exists(target_dir):
    #             os.makedirs(target_dir, exist_ok=True)
    #             logger.debug(f"[MainWindow] ðŸ“ Created target my_skills directory: {target_dir}")
    ...
    #                     logger.debug(f"[MainWindow] ðŸ”„ Overwriting existing example skill: {skill_name}")
    #                     try:
    ...
    #         if copied_count > 0:
    #             logger.info(f"[MainWindow] ðŸŽ‰ Successfully copied {copied_count} example skill(s) to my_skills directory")
    #         if updated_count > 0:
    #             logger.info(f"[MainWindow] ðŸ”„ Successfully updated {updated_count} example skill(s) in my_skills directory")
    #         if skipped_count > 0:
    #             logger.debug(f"[MainWindow] ðŸ“Š Skipped {skipped_count} existing skill(s)")
    #         if copied_count == 0 and updated_count == 0 and skipped_count == 0:
    #             logger.debug(f"[MainWindow] ðŸ“‚ No skills found to copy from {source_dir}")
                
    #     except Exception as e:
    #         logger.debug(f"[MainWindow] âŒ Error copying example my_skills: {e}")
    ...
    def _init_core_system(self, auth_manager, mainloop, ip, user, homepath, machine_role, schedule_mode):
        ...
        logger.info("[MainWindow] ðŸ”§ Initializing core system components...")
    ...
    def _init_system_info(self):
        ...
        logger.info("[MainWindow] ðŸ’» Initializing system information...")
        ...
        try:
            ...
            if isinstance(_mn, str):
                _mn = _mn.strip().strip('"').strip("'").replace("â€™", "'")
                _mn = " ".join(_mn.split())
                if "." in _mn:
                    _mn = _mn.split(".", 1)[0]
    ...
    def _init_file_system(self):
        ...
        logger.info("[MainWindow] ðŸ“ Initializing file system...")
    ...
    def _init_configuration_manager(self):
        ...
        logger.info("[MainWindow] âš™ï¸ Initializing configuration manager...")
    ...
    def _init_database(self):
        ...
        logger.info("[MainWindow] ðŸ—„ï¸ Initializing database ...")
        ...
        if "Commander" in self.machine_role:
            ...
            db_init_time = time.time() - start_time
            logger.info(f"[MainWindow] ðŸ—„ï¸ ecb Database engine created in {db_init_time:.3f}s")
    ...
    def _init_database_services(self):
        """Initialize database and related services with parallel optimization"""
        logger.info("[MainWindow] ðŸ—„ï¸ Initializing database services...")
    ...
    def _check_vehicles_with_database(self):
        ...
        logger.info("[MainWindow] ðŸš— Checking vehicles with database services...")
    ...
    def _load_default_template_data(self):
        """Load default template data for new users"""
        try:
            logger.info("[MainWindow] ðŸ“‹ Loading default template data...")
    ...
    def _load_organization_template(self):
        ...
        try:
            ...
            if result.get("success"):
                ...
                logger.error(f"[MainWindow] âŒ Failed to load organization template: {result.get('error')}")
            
        except Exception as e:
            logger.error(f"[MainWindow] âŒ Failed to load organization template: {e}")
            logger.error(traceback.format_exc())

    def _init_business_objects(self):
        """Initialize business objects and data structures"""
        logger.info("[MainWindow] ðŸ“Š Initializing business objects...")
    ...
    def _init_network_communication(self):
        ...
        logger.info("[MainWindow] ðŸŒ Initializing network communication...")
    ...
    def _init_local_data_loading(self):
        ...
        logger.info("[MainWindow] ðŸ“‚ Loading local data initialization...")
    ...
    def _init_async_tasks(self):
        ...
        logger.info("[MainWindow] âš¡ Initializing async tasks...")
    ...
    async def _async_sync_cloud_data(self):
        ...
        """
        try:
            logger.info("[MainWindow] ðŸŒ Starting background cloud data synchronization...")

            # Sync data
            logger.info("ðŸ“¥ Syncing data from cloud...")

            # Reload local missions after cloud sync

            # Update UI to reflect new data
            logger.info("[MainWindow] ðŸŽ‰ Background cloud data synchronization completed successfully!")
    ...
    def _startup_sync_offline_cloud_cache(self):
        ...
        """
        try:
            logger.info("[MainWindow] ðŸš€ Starting startup sync (blocking)...")
            ...
            if total_count > 0:
                logger.info(f"[MainWindow] ðŸ“¤ Found {total_count} tasks to sync ({pending_count} pending, {failed_count} failed)")
                ...
                def _sync_task():
                    ...
                    try:
                        start_time = time.time()
                        logger.info("[MainWindow] ðŸ”„ Starting startup sync in background (timeout: 15s per task)...")
    ...
    async def _async_setup_browser_manager(self):
        ...
        try:
            ...
            logger.info("[MainWindow] ðŸŒ Starting background browser manager initialization...")
    ...
    async def _async_start_account_notification_subscription(self):
        ...
        try:
            ...
            logger.info("[MainWindow] ðŸ“¢ Starting Account Notification Subscription...")
    ...
    async def _async_start_passive_command_subscription(self):
        ...
        try:
            ...
            err_msg = get_traceback(e, "ErrorStartPassiveCommandSubscription")
            logger.error(f"[MainWindow] {err_msg}")
    ...
    async def async_agents_init(self):
        ...
        try:
            logger.info("[MainWindow] ðŸš€ Starting ultra-optimized async agents initialization...")
            local_server_port = self.get_local_server_port()
            
            # Phase 1: Instant basic setup (target: <50ms)
            logger.info("[MainWindow] âš¡ Phase 1: Instant setup...")
            ...
            logger.info(f"[MainWindow] ðŸŽ¯ Adaptive MCP timeout: {mcp_timeout:.1f}s (CPU: {cpu_count}, RAM: {memory_gb:.1f}GB)")
            ...
            logger.info("[MainWindow] ðŸ”§ Starting pipelined agent initialization...")
            ...
            try:
                logger.info("[MainWindow] ðŸš€ Building and launching agents with ultra-parallel optimization...")
            ...
            logger.info("=" * 50)
            logger.info(f"â±ï¸  Total Time: {total_elapsed:.3f}s")
            logger.info(f"ðŸ“‹ Phase Breakdown:")
            ...
            logger.info(f"ðŸ’» System: CPU={cpu_count}, RAM={memory_gb:.1f}GB")
            logger.info("=" * 50)

        except Exception as e:
            total_elapsed = time.time() - start_time
            logger.error(f"[MainWindow] âŒ Optimized agents initialization failed after {total_elapsed:.3f}s: {e}")
            ...
            error_msg = f"Agent initialization failed: {str(e)}\n\n" \
                       ...
                       f"Possible causes:\n" \
                       f"â€¢ Server connection timeout\n" \
                       f"â€¢ Resource conflicts\n" \
                       f"â€¢ Configuration errors\n\n" \
    ...
    async def _wait_for_server_ready(self, local_server_port: int):
        ...
        logger.info(f"[MainWindow] ðŸ” Fast server check on port {local_server_port}...")
        host = "127.0.0.1"
    ...
    async def _build_agent_tasks_async(self):
        ...
        logger.info("[MainWindow] ðŸ“ Building agent tasks in parallel...")
    ...
    async def _obtain_agent_tools_async(self):
        ...
        try:
            ...
            logger.debug("[MainWindow] ðŸ› ï¸ Obtaining agent tools...")
            tools = obtain_agent_tools(self)
            return tools
        except Exception as e:
            logger.error(f"[MainWindow] âŒ Failed to obtain agent tools: {e}")
            raise
    ...
    async def _build_agent_knowledges_async(self):
        ...
        try:
            ...
            logger.debug("[MainWindow] ðŸ“š Building agent knowledges...")
            knowledges = build_agent_knowledges(self)
            return knowledges
        except Exception as e:
            logger.error(f"[MainWindow] âŒ Failed to build agent knowledges: {e}")
            raise
    ...
    async def _build_and_launch_agents_ultra_parallel(self):
        ...
        """
        try:
            logger.info("[MainWindow] ðŸš€ Starting ultra-parallel agent build and launch...")
            start_time = time.time()
            ...
            logger.info("[MainWindow] ðŸ”§ Starting 3-way parallel execution:")
            logger.info("[MainWindow]    1ï¸âƒ£ Building code-built agents")
            logger.info("[MainWindow]    2ï¸âƒ£ Loading local database agents")
            logger.info("[MainWindow]    3ï¸âƒ£ Fetching cloud agents")
            ...
            if isinstance(built_agents, Exception):
                logger.error(f"[MainWindow] âŒ Failed to build code agents: {built_agents}")
                built_agents = []
            if isinstance(local_db_agents, Exception):
                logger.error(f"[MainWindow] âŒ Failed to load local DB agents: {local_db_agents}")
                local_db_agents = []
            if isinstance(cloud_agents, Exception):
                logger.error(f"[MainWindow] âŒ Failed to fetch cloud agents: {cloud_agents}")
                cloud_agents = []
            
            logger.info(f"[MainWindow] ðŸ“Š Parallel loading completed:")
            ...
            logger.info("[MainWindow] ðŸ”€ Merging agents from all sources...")
            ...
            logger.info(f"[MainWindow] ðŸ“Š DB + Cloud merged: {len(merged_db_cloud)} agents")
            ...
            logger.info(f"[MainWindow] ðŸŽ‰ Ultra-parallel process completed in {total_time:.3f}s")
        ...
        try:
            ...
            async def build_single(config):
                try:
                    ...
                    logger.error(f"[MainWindow] âŒ Failed to build {config['name']}: {e}")
                    return None
            ...
            logger.error(f"[MainWindow] âŒ Failed to build code agents: {e}")
            return []
    ...
    def _launch_agents_async(self, agents_list):
        ...
        async def launch_all_async():
            ...
            logger.info(f"[MainWindow] ðŸš€ Launching {len(launchable_agents)} agents in background (fire-and-forget)...")
            ...
            logger.info(f"[MainWindow] ðŸŽ‰ Background launch completed: {launched_count}/{len(launchable_agents)} agents launched")
    ...
    async def _launch_single_agent_with_name_async(self, agent_name: str, agent):
        ...
        try:
            ...
            try:
                ...
                pass
            logger.error(f"[MainWindow] âŒ Failed to launch {agent_name}: {e}")
            logger.error(f"[MainWindow] Traceback: {traceback.format_exc()}")
            return False
    ...
    async def _load_local_db_agents(self):
        ...
        """
        try:
            logger.info("[MainWindow] ðŸ“‚ Loading agents from local database...")
            logger.info(f"[MainWindow] ðŸ“‚ Current user (original): {self.user}")
            ...
            logger.info(f"[MainWindow] ðŸ“‚ Agent service available: {agent_service}")
            ...
            def load_from_service():
                logger.info(f"[MainWindow] ðŸ“‚ Querying agents for owner: {self.user}")
                result = agent_service.get_agents_by_owner(self.user)
                logger.info(f"[MainWindow] ðŸ“‚ Query result: success={result.get('success')}, data_count={len(result.get('data', []))}")
                if result.get("success"):
                    agents_data = result.get("data", [])
                    logger.info(f"[MainWindow] ðŸ“‚ Found {len(agents_data)} agents in database")
                    return agents_data
                else:
                    logger.error(f"[MainWindow] âŒ Agent service query failed: {result.get('error')}")
                    return []
            ...
            logger.error(f"[MainWindow] âŒ Failed to load local DB agents: {e}")
            return []
    ...
    def _merge_agent_tasks_to_memory(self):
        ...
        """
        try:
            logger.info("[MainWindow] ðŸ“ Merging agent.tasks from built agents...")
            merged_count = 0
            skipped_count = 0
    ...
    async def _fetch_cloud_agents(self):
        ...
        """
        try:
            logger.info("[MainWindow] ðŸŒ Fetching agents from cloud...")
            ...
            logger.info(f"[MainWindow] ðŸ”„ Converted {len(cloud_agent_dicts)} cloud agents to dicts")
            return cloud_agent_dicts
            
        except Exception as e:
            logger.error(f"[MainWindow] âŒ Failed to fetch cloud agents: {e}")
            logger.error(f"[MainWindow] Traceback: {traceback.format_exc()}")
            return []
    ...
    def _merge_agent_data(self, local_agents, cloud_agents):
        ...
        """
        try:
            logger.info("[MainWindow] ðŸ”€ Merging local and cloud agent data...")
            ...
            logger.error(f"[MainWindow] âŒ Failed to merge agent data: {e}")
    ...
    def _merge_all_agent_sources(self, code_built_agents, db_cloud_agents):
        ...
        """
        try:
            logger.info("[MainWindow] ðŸ”€ Merging all agent sources into unified list...")
            ...
            for name, agent in code_built_agents:
                if agent:
                    ...
                    logger.debug(f"[MainWindow] ðŸ”§ Added code-built agent: {name}")
            ...
            logger.error(f"[MainWindow] âŒ Failed to merge all agent sources: {e}")
            import traceback
            logger.error(traceback.format_exc())
    ...
    async def _update_local_db_agents(self, merged_agents):
        ...
        try:
            ...
            logger.error(f"[MainWindow] âŒ Failed to update local DB agents: {e}")
    ...
    async def _start_webdriver_initialization(self):
        ...
        try:
            ...
            logger.info("ðŸš€ Starting WebDriver initialization...")
    ...
    async def processPlatoonMsgs(self, msgString, ip):
        ...
        try:
            ...
            if msg["type"] == "intro" or msg["type"] == "pong":
                ...
                # update mission status to the cloud and to local data structure and to chatï¼Ÿ
                # "mid": mid,
