...
def lan_http_request2(query_js, imgs, session, token, lan_endpoint):
    ...
    with httpx.Client(timeout=timeout, event_hooks={"request": [lambda r: logger.debug(f"🔍 Raw HTTP Request: {r}") ]}) as client:
        try:
        ...
        try:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    break
                except (httpx.ReadError, httpx.RemoteProtocolError, httpx.ConnectError, httpx.ReadTimeout, httpx.WriteTimeout) as e:
                    last_error = e
                    ...
                    if attempt < max_retries - 1:
                        ...
                        logger.error(f"Endpoint: {LAN_API_ENDPOINT_URL}, Request size: {total_size / 1024 / 1024:.2f} MB")
                        logger.error(f"Possible causes: 1) Server issues 2) Network instability 3) Request too large 4) Server timeout")
                        raise
                except Exception as e:
