...
def genWinADSETSYBrowserFullfillOrdersSkill(worksettings, stepN, theme):
    print("genWinADSETSYBrowserFullfillOrdersSkill")
    psk_words = "{"

    this_step, step_words = genStepHeader("win_ads_etsy_browser_fullfill_orders", "win", "1.0", "AIPPS LLC", "PUBWINADSETSY011",
                                          "Selenium Etsy Fullfill New Orders On Windows ADS.", stepN)
    psk_words = psk_words + step_words

    this_step, step_words = genStepStub("start skill main", "public/win_ads_etsy_orders/browser_fullfill_orders", "", this_step)
    psk_words = psk_words + step_words
    ...
    # this_step, step_words = genWinADSETSYBrowserInitializeSetup(worksettings, this_step, theme)
    # psk_words = psk_words + step_words

    this_step, step_words = genStepWebdriverGoToTab("web_driver", "etsy", "https://www.etsy.com/sh/ord/?filter=status:AWAITING_SHIPMENT", "site_result", "site_flag", this_step)
    psk_words = psk_words + step_words
    ...
    this_step, step_words = genStepUseSkill("browser_collect_orders", "public/win_ads_etsy_orders", "dummy_in", "dummy_out", this_step)
    psk_words = psk_words + step_words
    ...
    this_step, step_words = genStepCreateData("expr", "buy_shipping_input", "NA", "['sale', etsy_orders, product_book]", this_step)
    psk_words = psk_words + step_words
    #
    # using etsy to purchase shipping label will auto update tracking code..... so no need to do update tracking afterwords.
    this_step, step_words = genStepUseSkill("browser_buy_shipping", "public/win_ads_etsy_orders", "buy_shipping_input", "labels_dir", this_step)
    psk_words = psk_words + step_words
    ...
    this_step, step_words = genStepCreateData("expr", "reformat_print_input", "NA", "['one page', 'labels_dir', printer_name, etsy_orders, product_catelog]", this_step)
    psk_words = psk_words + step_words
    ...
    this_step, step_words = genStepStub("end skill", "public/win_ads_etsy_orders/browser_fullfill_orders", "", this_step)
    psk_words = psk_words + step_words
    ...
    return [
        {
            ...
            'dir': 'C:/Users/songc/PycharmProjects/ecbot/runlogs/songc_yahoo/20240815/b88m697/win_ads_etsy_orders/skills/browser_fullfill_orders_with_ecb_labels/ecb_labels',
            'file': 'etsyOrdersGround20240815122835.xls',
            'unzipped_dir': 'C:/Users/songc/PycharmProjects/ecbot/runlogs/songc_yahoo/20240815/b88m697/win_ads_etsy_orders/skills/browser_fullfill_orders_with_ecb_labels/ecb_labels/etsyOrdersGround20240815122835',
            'order_data': [
                {
                    "name":"",
                    "order_ids": [""],
...
def genWinADSETSYBrowserFullfillOrdersWithECBLabelsSkill(worksettings, stepN, theme):
    print("genWinADSETSYBrowserFullfillOrdersWithECBLabelsSkill")
    psk_words = "{"

    this_step, step_words = genStepHeader("win_ads_etsy_browser_fullfill_orders", "win", "1.0", "AIPPS LLC",
                                          "PUBWINADSETSY011",
                                          "Selenium Etsy Fullfill New Orders On Windows ADS.", stepN)
    psk_words = psk_words + step_words

    this_step, step_words = genStepStub("start skill main", "public/win_ads_etsy_orders/browser_fullfill_orders_with_ecb_labels", "", this_step)
    psk_words = psk_words + step_words
    ...
    this_step, step_words = genStepPrepareGSOrder("etsy_orders", "gs_orders", "product_book", "current_seller", "etsy", "fdir", this_step)
    psk_words = psk_words + step_words
    ...
    psk_words = psk_words + step_words
    #
    # using etsy to purchase shipping label will auto update tracking code..... s
    ...
    # "from datetime import datetime\nglobal hf_name\nhf_name= 'etsyOrders'+ str(int(datetime.now().timestamp()))
    ...
    this_step, step_words = genStepCreateData("expr", "update_tracking_input", "NA", "['sale', gs_orders, etsy_orders, product_book]", this_step)
    psk_words = psk_words + step_words

    this_step, step_words = genStepUseSkill("browser_update_tracking", "public/win_ads_etsy_orders", "update_tracking_input", "update_tracking_result", this_step)
    psk_words = psk_words + step_words
    # #
    # this_step, step_words = genStepCreateData("expr", "reformat_print_input", "NA", "['one page', 'labels_dir', printer_name, etsy_orders, product_catelog]", this_step)
    # psk_words = psk_words + step_words
    ...
    psk_words = psk_words + step_words
    #
    # # close the browser and exit the skill, assuming at the end of genWinCHROMEETSYWalkSteps, the browser tab
    # # should return to top of the etsy home page with the search text box cleared.
    ...
    this_step, step_words = genStepStub("end skill", "public/win_ads_etsy_orders/browser_fullfill_orders_with_ecb_labels", "",
                                        this_step)
    psk_words = psk_words + step_words
    print("generating win ads etsy skill")
    psk_words = psk_words + "\"dummy\" : \"\"}"
...
def genWinADSETSYBrowserBuyShippingSkill(worksettings, stepN, theme):
    print("genWinADSETSYBrowserBuyShippingSkill")
    psk_words = "{"

    this_step, step_words = genStepHeader("win_ads_etsy_browser_buy_shipping", "win", "1.0", "AIPPS LLC",
                                          "PUBWINADSETSY011",
                                          "Selenium Etsy buy shipping labels on etsy On Windows ADS.", stepN)
    psk_words = psk_words + step_words

    this_step, step_words = genStepStub("start skill main", "public/win_ads_etsy_orders/browser_buy_shipping", "", this_step)
    psk_words = psk_words + step_words
    ...
    this_step, step_words = genStepWebdriverGoToTab("web_driver", "etsy", "https://www.etsy.com/gslblui/bulk?_trkparms=lblmgmt", "site_result", "site_flag", this_step)
    psk_words = psk_words + step_words
    ...
    this_step, step_words = genStepStub("end skill", "public/win_ads_etsy_orders/browser_buy_shipping", "", this_step)
    psk_words = psk_words + step_words
    print("generating win ads etsy skill")
    psk_words = psk_words + "\"dummy\" : \"\"}"
...
def genWinADSETSYBrowserBuyECBLabelsSkill(worksettings, stepN, theme):
    print("genWinADSETSYBrowserBuyECBLabelsSkill")
    psk_words = "{"

    this_step, step_words = genStepHeader("win_ads_etsy_browser_buy_ecb_labels", "win", "1.0", "AIPPS LLC",
                                          "PUBWINADSETSY031",
                                          "In-Browser Etsy Buy ECB labels using external skill.", stepN)
    psk_words = psk_words + step_words

    this_step, step_words = genStepStub("start skill main", "public/win_ads_etsy_orders/browser_buy_ecb_labels", "",
                                        this_step)
    psk_words = psk_words + step_words
    ...
    this_step, step_words = genStepCreateData("expr", "buy_shipping_input", "NA", "['sale', etsy_orders, product_catelog]", this_step)
    psk_words = psk_words + step_words
    ...
    # now assume the result available in "order_track_codes" which is a list if [{"oid": ***, "sc": ***, "service": ***, "code": ***}]
    # now update tracking coded back to the orderlist
    this_step, step_words = genStepUseSkill("update_tracking", "public/win_ads_etsy_orders", "gs_input", "total_label_cost", this_step)
    psk_words = psk_words + step_words

    this_step, step_words = genStepCreateData("expr", "reformat_print_input", "NA",
                                              "['one page', 'labels_dir', printer_name, etsy_orders, product_catelog]",
                                              this_step)
    psk_words = psk_words + step_words
    ...
    this_step, step_words = genStepStub("end skill", "public/win_ads_etsy_orders/browser_buy_ecb_labels", "", this_step)
    psk_words = psk_words + step_words
    print("generating win ads etsy skill")
    psk_words = psk_words + "\"dummy\" : \"\"}"
    log3("DEBUG", "generated skill for windows etsy order browser fullfill operation...." + psk_words)
...
def genWinADSETSYBrowserInitializeSetup(worksettings, stepN, theme):
    psk_words = ""

    this_step, step_words = genStepCreateData("string", "etsy_status", "NA", "", stepN)
    psk_words = psk_words + step_words
    ...
    this_step, step_words = genStepCreateData("obj", "etsy_orders", "NA", [], this_step)
    psk_words = psk_words + step_words
    ...
    this_step, step_words = genStepCreateData("str", "web_driver_options", "NA", '', this_step)
    psk_words = psk_words + step_words
    ...
    this_step, step_words = genStepCallExtern( "global ads_profile_id\nprint('ads_profile_id =', ads_profile_id)", "", "in_line", "", this_step)
    psk_words = psk_words + step_words

    this_step, step_words = genStepCreateData("expr", "ads_port", "NA", "sk_work_settings['fp_browser_settings']['ads_port']", this_step)
    psk_words = psk_words + step_words

    this_step, step_words = genStepCreateData("expr", "ads_api_key", "NA", "sk_work_settings['fp_browser_settings']['ads_api_key']", this_step)
    psk_words = psk_words + step_words
    ...
    this_step, step_words = genStepCreateData("bool", "web_driver_successful", "NA", False, this_step)
    psk_words = psk_words + step_words
...
def genWinADSETSYBrowserCollectOrdersSkill(worksettings, stepN, theme):
    psk_words = "{"

    this_step, step_words = genStepHeader("win_ads_etsy_browser_collect_orders", "win", "1.0", "AIPPS LLC", "PUBWINADSETSY021",
                                          "in Browser Etsy Collect New Orders On Windows.", stepN)
    psk_words = psk_words + step_words

    this_step, step_words = genStepStub("start skill", "public/win_ads_etsy_orders/browser_collect_orders", "", this_step)
    psk_words = psk_words + step_words
    ...
    this_step, step_words = genStepCallExtern("from datetime import datetime\nglobal hf_name\nhf_name= 'etsyOrders'+ str(int(datetime.now().timestamp()))+'.html'\nprint('hf_name:',hf_name,datetime.now())", "", "in_line", "", this_step)
    psk_words = psk_words + step_words
    ...
    this_step, step_words = genStepCallExtern("global etsy_orders, n_orders_collected\netsy_orders.append(page_of_orders)\nn_orders_collected = n_orders_collected + len(page_of_orders['ol'])\nprint('n_orders_collected', n_orders_collected, len(etsy_orders))", "", "in_line", "", this_step)
    psk_words = psk_words + step_words
    ...
    this_step, step_words = genStepStub("end skill", "public/win_ads_etsy_orders/browser_collect_orders", "", this_step)
    psk_words = psk_words + step_words
...
def genWinCHROMEETSYBrowserCollectOrderListSkill(worksettings, stepN, theme):
    psk_words = "{"

    this_step, step_words = genStepHeader("win_chrome_etsy_browser_collect_orders", "win", "1.0", "AIPPS LLC", "PUBWINCHROMEETSY002",
                                          "Ebay Collect New Orders On Windows.", stepN)
    psk_words = psk_words + step_words

    this_step, step_words = genStepStub("start skill", "public/win_chrome_etsy_orders/browser_collect_orders", "", this_step)
    psk_words = psk_words + step_words

    this_step, step_words = genStepStub("end skill", "public/win_chrome_etsy_orders/browser_collect_orders", "", this_step)
    psk_words = psk_words + step_words
...
def genWinCHROMEETSYBuyShippingSkill(worksettings, stepN, theme):
    ...
    this_step, step_words = genStepHeader("win_chrome_etsy_browser_buy_shipping", "win", "1.0", "AIPPS LLC",
                                          "PUBWINCHROMEETSY013",
                                          "in Browser Etsy Buy Shipping On Windows Chrome.", stepN)
    psk_words = psk_words + step_words

    this_step, step_words = genStepStub("start skill", "public/win_chrome_etsy_orders/browser_buy_shipping", "", this_step)
    psk_words = psk_words + step_words

    this_step, step_words = genStepStub("end skill", "public/win_chrome_etsy_orders/browser_buy_shipping", "", this_step)
    psk_words = psk_words + step_words
...
def genWinADSETSYBuyShippingSkill(worksettings, stepN, theme):
    ...
    this_step, step_words = genStepHeader("win_ads_etsy_buy_shipping", "win", "1.0", "AIPPS LLC",
                                          "PUBWINCHROMEETSY012",
                                          "in Browser Etsy Buy Shipping and Update Tracking On Windows Chrome.", stepN)
    psk_words = psk_words + step_words

    this_step, step_words = genStepStub("start skill", "public/win_ads_etsy_orders/browser_buy_shipping", "", this_step)
    psk_words = psk_words + step_words


    # input ['sale', sender, etsy_orders, product_catelog]
    this_step, step_words = genStepCallExtern("global ship_op\nship_op = fin[0]", "", "in_line", "", this_step)
    psk_words = psk_words + step_words

    this_step, step_words = genStepCallExtern("global labels_dir\nlabels_dir = "+worksettings+"['log_path_prefix']+'etsy_labels'", "", "in_line", "", this_step)
    psk_words = psk_words + step_words
    ...
    # now create all label in bulk here: https://www.etsy.com/gslblui/bulk?_trkparms=lblmgmt
    this_step, step_words = genStepCallExtern("global bulkurl\nbulkurl = 'https://www.etsy.com/gslblui/bulk?_trkparms=lblmgmt'", "", "in_line", "", this_step)
    psk_words = psk_words + step_words
    ...
    this_step, step_words = genStepSearchAnchorInfo("screen_info", "ground_advantage", "direct", "anchor text", "any", "ga_locs", "used_ground_advantage", "etsy", False, this_step)
    psk_words = psk_words + step_words
    ...
    psk_words = psk_words + step_words
    # 0.73
    this_step, step_words = genScrollDownUntilLoc(["materials"], "anchor text", 80, "etsy_bulk_labels", "top", "scroll_adjustment",
    ...
    # this_step, step_words = genStepSearchAnchorInfo("screen_info", ["review_purchase"], "direct", ["anchor text"], "any", "complete_buttons", "useless", "etsy", False, this_step)
    # psk_words = psk_words + step_words
    ...
    # use this info, as it contains the name and address, as well as the ship_to anchor location.
    this_step, step_words = genStepSearchAnchorInfo("screen_info", ["confirm_and_pay"], "direct", ["anchor text"], "any", "complete_buttons", "useless", "etsy", False, this_step)
    psk_words = psk_words + step_words
    ...
    # now go the default download directory and fetch the most recent "etsy-bulk-labels-*.pdf"
    this_step, step_words = genStepMoveDownloadedFileToDestination("etsy-bulk-labels", "pdf", "labels_dir", "move_done", this_step)
    psk_words = psk_words + step_words
    ...
    psk_words = psk_words + "\"dummy\" : \"\"}"
    log3("DEBUG", "generated skill for windows chrome in browser etsy buy shipping and update tracking...." + psk_words)
...
def genWinADSETSYBrowserUpdateTrackingSkill(worksettings, stepN, theme):
    psk_words = "{"


    this_step, step_words = genStepHeader("win_ads_etsy_fullfill_orders", "win", "1.0", "AIPPS LLC", "PUBWINADSETSY001",
                                          "in Browser Etsy Fullfill New Orders On Windows.", stepN)
    psk_words = psk_words + step_words

    this_step, step_words = genStepStub("start skill", "public/win_ads_etsy_orders/browser_update_tracking", "", this_step)
    psk_words = psk_words + step_words

    # open the order page again.
    this_step, step_words = genStepCallExtern("global blurl\nblurl = 'https://www.etsy.com/sh/ord/?filter=status:AWAITING_SHIPMENT'", "", "in_line", "",
                                              this_step)
    psk_words = psk_words + step_words

    this_step, step_words = genStepsWinCHROMEETSYBrowserUpdateTracking(worksettings, this_step, theme)
    psk_words = psk_words + step_words


    this_step, step_words = genStepStub("end skill", "public/win_ads_etsy_orders/browser_update_tracking", "", this_step)
    psk_words = psk_words + step_words

    psk_words = psk_words + "\"dummy\" : \"\"}"
    log3("DEBUG", "generated skill for windows chrome in browser update tracking info on etsy...." + psk_words)

    return this_step, psk_words


def genWinCHROMEETSYBrowserUpdateTrackingSkill(worksettings, stepN, theme):
    psk_words = "{"
    ...
    this_step, step_words = genStepStub("start skill", "public/win_chrome_etsy_orders/browser_update_tracking", "", this_step)
    psk_words = psk_words + step_words
    ...
    this_step, step_words = genStepStub("end skill", "public/win_chrome_etsy_orders/browser_update_tracking", "", this_step)
    psk_words = psk_words + step_words
...
def genStepsWinCHROMEETSYBrowserUpdateTracking(worksettings, stepN, theme):
    psk_words = ""
    ...
    this_step, step_words = genStepCreateData("obj", "in_etsy_orders", "NA", None, this_step)
    psk_words = psk_words + step_words
    ...
    this_step, step_words = genStepCallExtern("global fin, in_etsy_orders, in_gs_labels, in_prod_book\nin_gs_orders = fin[1]\nin_etsy_orders = fin[2]\nin_prod_book = fin[3]", "", "in_line", "", this_step)
    psk_words = psk_words + step_words
    ...
    this_step, step_words = genStepCallExtern("global numOrders, in_etsy_orders\nnumOrders = sum(len(page['ol']) for page in in_etsy_orders)\nprint('numOrders', numOrders, in_etsy_orders[0]['orders_per_page'])", "", "in_line", "", this_step)
    psk_words = psk_words + step_words
    ...
    # obtain tracking code of nth order, since etsy orders are organized as pages. first need to figure out which page is it on, and then
    # the nth element on that page. use the  declared ealier

    this_step, step_words = genStepCallExtern("global tracking_code, nOrdersUpdated, nth_page, mth_item\nnth_page=math.floor(nOrdersUpdated/etsy_orders[0]['orders_per_page'])\nmth_item=nOrdersUpdated%etsy_orders[0]['orders_per_page']\ntracking_code=etsy_orders[nth_page]['ol'][mth_item].getShippingTracking()\nprint('tracking_code is:', tracking_code, nth_page, mth_item, nOrdersUpdated, etsy_orders[0]['orders_per_page'])", "", "in_line", "", this_step)
    psk_words = psk_words + step_words
...
def genWinADSETSYBrowserRespondMessagesSkill(worksettings, stepN, theme):
    psk_words = "{"

    this_step, step_words = genStepHeader("win_ads_etsy_handle_messages", "win", "1.0", "AIPPS LLC",
                                          "PUBWINADSETSY002",
                                          "in Browser Etsy Buy Shipping and Update Tracking On Windows ADS.", stepN)
    psk_words = psk_words + step_words

    this_step, step_words = genStepStub("start skill", "public/win_ads_etsy_orders/browser_handle_messages", "", this_step)
    psk_words = psk_words + step_words

    this_step, step_words = genStepStub("end skill", "public/win_ads_etsy_orders/browser_handle_messages", "", this_step)
    psk_words = psk_words + step_words

    psk_words = psk_words + "\"dummy\" : \"\"}"
    log3("DEBUG", "generated skill for windows ads in browser etsy handle messages...." + psk_words)

    return this_step, psk_words


def genWinCHROMEETSYBrowserRespondMessagesSkill(worksettings, stepN, theme):
    psk_words = "{"
    ...
    this_step, step_words = genStepStub("start skill", "public/win_chrome_etsy_orders/browser_handle_messages", "", this_step)
    psk_words = psk_words + step_words
    ...
    this_step, step_words = genStepStub("end skill", "public/win_chrome_etsy_orders/browser_handle_messages", "", this_step)
    psk_words = psk_words + step_words

    psk_words = psk_words + "\"dummy\" : \"\"}"
    log3("DEBUG", "generated skill for windows chrome in browser etsy handle messages...." + psk_words)

    return this_step, psk_words


# buy and download labels from EBAY using USPS, Steps:
#  1) go to https://www.etsy.com/gslblui/bulk/ for bulk purchase.
...
#  note: one advantage is purchasing labels from etsy will auto update tracking, so this step
#        is saved. the disadvantge is labels might not be the cheapest.
def genWinADSETSYBuyShippingLabelsSkill(worksettings, stepN, theme):
    psk_words = "{"

    this_step, step_words = genStepHeader("win_ads_etsy_buy_labelss", "win", "1.0", "AIPPS LLC", "PUBWINADSETSY003",
                                          "Ebay Buy Shipping On Windows.", stepN)
    psk_words = psk_words + step_words

    this_step, step_words = genStepStub("start skill", "public/win_ads_etsy_orders/buy_labels", "", this_step)
    psk_words = psk_words + step_words

    this_step, step_words = genStepCreateData("obj", "sk_work_settings", "NA", worksettings, this_step)
    psk_words = psk_words + step_words

    this_step, step_words = genStepCallExtern("global blurl\nblurl = 'https://www.etsy.com/gslblui/bulk/'", "", "in_line", "",
                                              this_step)
    psk_words = psk_words + step_words
    ...
    this_step, step_words = genStepStub("end skill", "public/win_ads_etsy_orders/buy_labels", "", this_step)
    psk_words = psk_words + step_words

    psk_words = psk_words + "\"dummy\" : \"\"}"
    log3("DEBUG", "generated skill for windows ads etsy purchase shipping labels...." + psk_words)

    return this_step, psk_words


def genWinADSETSYHandleReturnsSkill(worksettings, stepN, theme):
    psk_words = "{"

    this_step, step_words = genStepHeader("win_ads_etsy_handle_returns", "win", "1.0", "AIPPS LLC", "PUBWINADSETSY001",
                                          "Ebay Handle Return Orders On Windows.", stepN)
    psk_words = psk_words + step_words

    this_step, step_words = genStepStub("start skill main", "public/win_ads_etsy_orders/handle_returns", "", this_step)
    psk_words = psk_words + step_words
    ...
    this_step, step_words = genStepStub("end skill", "public/win_ads_etsy_orders/handle_returns", "", this_step)
    psk_words = psk_words + step_words

    psk_words = psk_words + "\"dummy\" : \"\"}"
    log3("DEBUG", "generated skill for windows etsy handle return operation...." + psk_words)

    return this_step, psk_words



def genWinADSETSYHandleMsgsSkill(worksettings, stepN, theme):
    psk_words = "{"

    this_step, step_words = genStepHeader("win_ads_etsy_handle_msgs", "win", "1.0", "AIPPS LLC", "PUBWINADSETSY002",
    ...
    #
    # this_step, step_words = genStepThink("end skill", "public/win_ads_etsy_orders/handle_returns", "", this_step)
    # psk_words = psk_words + step_words
...
def genEbayLoginInSteps(stepN, theme):
    ...
    this_step, step_words = genStepSearchAnchorInfo("screen_info", "etsy_logo0", "direct", "anchor icon", "any", "useless", "site_loaded", "", False, this_step)
    psk_words = psk_words + step_words

    this_step, step_words = genStepCheckCondition("site_loaded", "", "", this_step)
    psk_words = psk_words + step_words

    this_step, step_words = genStepMouseClick("Single Click", "", True, "screen_info", "etsy_logo0", "anchor icon", "", [0, 0], "center", [0, 0], "box", 1, 1, [0, 0], this_step)
    psk_words = psk_words + step_words
    ...
    this_step, step_words = genStepTextInput("text", False, "https://www.etsy.com/sh/ord/?filter=status:AWAITING_SHIPMENT", "direct", 0.05, "enter", 1, this_step)
    psk_words = psk_words + step_words
    ...
    this_step, step_words = genStepExtractInfo("", "sk_work_settings", "screen_info", "etsy_home", "top", theme, this_step, None)
    psk_words = psk_words + step_words
...
def genLabelName(oinfo, first_name, last_name):
    label_file_name = "etsy_" + first_name + "_" + last_name + "_"
...
def genWinADSETSYRespondMessagesSkill(worksettings, stepN, theme):
    psk_words = "{"

    this_step, step_words = genStepHeader("win_ads_etsy_respond_messages", "win", "1.0", "AIPPS LLC", "PUBWINADSETSY005",
                                          "Ebay Respond To Customer Messages On Windows ADS.", stepN)
    psk_words = psk_words + step_words

    this_step, step_words = genStepStub("start skill main", "public/win_ads_etsy_orders/respond_messages", "", this_step)
    psk_words = psk_words + step_words


    # now create all label in bulk here: https://www.etsy.com/gslblui/bulk?_trkparms=lblmgmt
    this_step, step_words = genStepCallExtern("global msgurl\nmsgurl = 'https://mesg.etsy.com/mesgweb/ViewMessages/0'", "", "in_line", "", this_step)
    psk_words = psk_words + step_words
    ...
    this_step, step_words = genStepStub("end skill", "public/win_ads_etsy_orders/respond_messages", "", this_step)
    psk_words = psk_words + step_words
...
def genWinADSETSYBrowserRespondMessagesSkill(worksettings, stepN, theme):
    ...
    this_step, step_words = genStepHeader("win_ads_etsy_browser_respond_messages", "win", "1.0", "AIPPS LLC", "PUBWINADSETSY005",
                                          "Ebay Respond To Customer Messages On Windows ADS Using In-Browser Automation Skills.", stepN)
    psk_words = psk_words + step_words

    this_step, step_words = genStepStub("start skill main", "public/win_ads_etsy_orders/browser_respond_messages", "", this_step)
    psk_words = psk_words + step_words
    ...
    # this_step, step_words = genStepWebdriverGoToTab("web_driver", "etsy", "https://mesg.etsy.com/mesgweb/ViewMessages/0/All", "site_result", "site_flag", this_step)
    # this_step, step_words = genStepWebdriverGoToTab("web_driver", "etsy", "https://mesg.etsy.com/mesgweb/ViewMessages/0/etsy", "site_result", "site_flag", this_step)

    # 1st handle member messages
    this_step, step_words = genStepWebdriverGoToTab("web_driver", "etsy", "https://mesg.etsy.com/mesgweb/ViewMessages/0/m2m", "site_result", "site_flag", this_step)
    psk_words = psk_words + step_words
    ...
    this_step, step_words = genStepStub("end skill", "public/win_ads_etsy_orders/browser_respond_messages", "", this_step)
    psk_words = psk_words + step_words

    psk_words = psk_words + "\"dummy\" : \"\"}"
    log3("DEBUG", "generated skill for windows etsy handle customer and system messages using in-browser automation skills...." + psk_words)

    return this_step, psk_words



def genWinADSETSYBrowserHandleOffersSkill(worksettings, stepN, theme):
    psk_words = "{"

    this_step, step_words = genStepHeader("win_ads_etsy_browser_handle_offers", "win", "1.0", "AIPPS LLC", "PUBWINADSETSY015",
                                          "In-Browser Etsy Respond To Customer Offers On Windows ADS.", stepN)
    psk_words = psk_words + step_words

    this_step, step_words = genStepStub("start skill main", "public/win_ads_etsy_orders/browser_handle_offers", "", this_step)
    psk_words = psk_words + step_words
    ...
    this_step, step_words = genStepStub("end skill", "public/win_ads_etsy_orders/browser_handle_offers", "", this_step)
    psk_words = psk_words + step_words

    psk_words = psk_words + "\"dummy\" : \"\"}"
    log3("DEBUG", "generated skill for windows etsy handle respoding to offers using in-browser automation skills...." + psk_words)

    return this_step, psk_words

def genWinADSETSYBrowserHandleReturnSkill(worksettings, stepN, theme):
    psk_words = "{"

    this_step, step_words = genStepHeader("win_ads_etsy_browser_handle_return", "win", "1.0", "AIPPS LLC", "PUBWINADSETSY005",
                                          "Ebay Respond To Customer Messages On Windows ADS.", stepN)
    psk_words = psk_words + step_words

    this_step, step_words = genStepStub("start skill main", "public/win_ads_etsy_orders/browser_handle_return", "", this_step)
    psk_words = psk_words + step_words
    ...
    this_step, step_words = genStepStub("end skill", "public/win_ads_etsy_orders/browser_handle_return", "", this_step)
    psk_words = psk_words + step_words

    psk_words = psk_words + "\"dummy\" : \"\"}"
    log3("DEBUG", "generated skill for windows etsy handle return with etsy labels using in-browser automation skills...." + psk_words)

    return this_step, psk_words



def genWinADSETSYBrowserHandleReturnWithECBLabelsSkill(worksettings, stepN, theme):
    psk_words = "{"

    this_step, step_words = genStepHeader("win_ads_etsy_browser_handle_return_with_ecb_labels", "win", "1.0", "AIPPS LLC", "PUBWINADSETSY005",
                                          "In-Browser Etsy handles return with ecb labels On Windows ADS.", stepN)
    psk_words = psk_words + step_words

    this_step, step_words = genStepStub("start skill main", "public/win_ads_etsy_orders/browser_handle_return_with_ecb_labels", "", this_step)
    psk_words = psk_words + step_words
    ...
    this_step, step_words = genStepStub("end skill", "public/win_ads_etsy_orders/browser_handle_return_with_ecb_labels", "", this_step)
    psk_words = psk_words + step_words

    psk_words = psk_words + "\"dummy\" : \"\"}"
    log3("DEBUG", "generated skill for windows etsy handle return with ECB generated labels using in-browser automation skills...." + psk_words)

    return this_step, psk_words

def genWinADSETSYBrowserHandleReplacementSkill(worksettings, stepN, theme):
    psk_words = "{"

    this_step, step_words = genStepHeader("win_ads_etsy_browser_handle_replacement", "win", "1.0", "AIPPS LLC", "PUBWINADSETSY005",
                                          "Ebay Respond To Customer Messages On Windows ADS.", stepN)
    psk_words = psk_words + step_words

    this_step, step_words = genStepStub("start skill main", "public/win_ads_etsy_orders/browser_handle_replacement", "", this_step)
    psk_words = psk_words + step_words
    ...
    this_step, step_words = genStepStub("end skill", "public/win_ads_etsy_orders/browser_handle_replacement", "", this_step)
    psk_words = psk_words + step_words

    psk_words = psk_words + "\"dummy\" : \"\"}"
    log3("DEBUG", "generated skill for windows etsy handle sending replacement using in-browser automation skills...." + psk_words)

    return this_step, psk_words

def genWinADSETSYBrowserHandleRefundSkill(worksettings, stepN, theme):
    psk_words = "{"

    this_step, step_words = genStepHeader("win_ads_etsy_browser_handle_refund", "win", "1.0", "AIPPS LLC", "PUBWINADSETSY005",
                                          "Ebay Respond To Customer Messages On Windows ADS.", stepN)
    psk_words = psk_words + step_words

    this_step, step_words = genStepStub("start skill main", "public/win_ads_etsy_orders/browser_handle_refund", "", this_step)
    psk_words = psk_words + step_words
    ...
    this_step, step_words = genStepStub("end skill", "public/win_ads_etsy_orders/browser_handle_refund", "", this_step)
    psk_words = psk_words + step_words
