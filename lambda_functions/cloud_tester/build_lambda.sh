#!/bin/bash
#
# build_lambda.sh - Build and deploy cloud_tester Lambda
#
# Usage:
...
echo ""
echo "  # Test C2C_WS_Test (Cloud to Cloud/Web):"
echo '  aws lambda invoke --function-name cloud_tester \'
echo "    --profile $AWS_PROFILE --region $REGION \\"
echo '    --payload '"'"'{"name":"C2C_WS_Test","input":{"owner":"YOUR_OWNER","acctSiteID":"YOUR_SITE"}}'"'"' \'
echo '    --cli-binary-format raw-in-base64-out \'
echo '    response.json && cat response.json'
echo ""
echo "  # Test C2L_WS_Test (Cloud to Local):"
echo '  aws lambda invoke --function-name cloud_tester \'
echo "    --profile $AWS_PROFILE --region $REGION \\"
echo '    --payload '"'"'{"name":"C2L_WS_Test","input":{"owner":"YOUR_OWNER","acctSiteID":"YOUR_SITE"}}'"'"' \'
echo '    --cli-binary-format raw-in-base64-out \'
echo '    response.json && cat response.json'
echo ""
