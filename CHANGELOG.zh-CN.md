...
### 修复
- **浏览器自动化**：修复了打包应用中自带浏览器缺失的问题，确保开箱即用。
- **Windows 兼容性**：修复了在 Windows 上使用 browser-use 的 "new chromium" 模式时无法启动子进程的问题（解决了 PySide6 事件循环冲突）。
- **浏览器插件**：修复了构建过程中浏览器插件下载失败的问题，确保扩展功能正常。
