...
6. **ALSO register in the desktop `tool_function_mapping`** in `agent/mcp/server/server.py` (line ~3807). This is what the desktop runtime's `unified_tool_handler` consults. Skipping this step makes the tool fail at runtime with `Tool 'X' not found in tool_function_mapping` even though everything else looks wired correctly. The two registries are parallel — _CLOUD_TOOL_REGISTRY is for the cloud-worker fast path, tool_function_mapping is for the desktop MCP server.
