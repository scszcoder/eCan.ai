...
**`gui/` 目录是废弃目录** — 这是旧的 PyQt5 GUI 的 Python 代码，现在已被 `gui_v2/` (React/TypeScript) 替换。
...
| 文件 | 用途 | 读取方式 |
|------|------|----------|
| `auth_config.yml` APPSYNC.* | **后端代码调用云端 GraphQL** | `agent/cloud_api/endpoints.py` |
| `cloud_endpoints.json` | **存储/存储桶配置** | `utils/app_config_loader.py` |

### 详细说明

**`apps/{cn,intl}/config/auth_config.yml`** — 后端调用云端 API 的真相源：
```yaml
APPSYNC:
  GRAPHQL_ENDPOINT: https://sccb0-d0gc5398xf028be6a.service.tcloudbase.com/api/graphql
  WS_ENDPOINT: wss://ecan-graphql-ws-xxx.sh.run.tcloudbase.com/ws
```
- 后端代码（`agent/cloud_api/*.py`）调用云端 API 使用此配置
- 包含私密信息（如有 API Key）

**`apps/{cn,intl}/config/cloud_endpoints.json`** — 存储和存储桶配置：
```json
{
  "graphql": "https://api.fastprecisiontech.com/graphql",
  "websocket": "wss://ws.fastprecisiontech.com/graphql",
...
- **构建时**：两个配置文件都打包进应用
- **运行时**：
  - 后端 API 调用 → `auth_config.yml`（通过 `get_endpoint_config()`）
  - 存储操作 → `cloud_endpoints.json`（通过 `AppConfigLoader`）

## 9. Release Pipeline Invariants
