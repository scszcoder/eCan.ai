...
def set_up_ec_marketing_agent(mainwin):
    try:
        ...
        if not worker_skill:
            logger.error("Skill 'ecbot rpa marketing' not found! Aborting setup.")
            return None
        if not chatter_skill:
            logger.error("Skill 'ecbot rpa marketing chatter' not found! Aborting setup.")
            return None

        agent_card = AgentCard(
            id="ec_marketing_agent",
        ...
        logger.info("ec_marketing agent card created:", agent_card.name, agent_card.url)

        chatter_task = create_ec_marketing_chat_task(mainwin)
        if not chatter_task:
            logger.error("Failed to create chatter task for ec_marketing! Aborting setup.")
            return None
            
        worker_task = create_ec_marketing_work_task(mainwin)
        if not worker_task:
            logger.error("Failed to create worker task for ec_marketing! Aborting setup.")
            return None
            
        # Use mainwin's unified browser_use_llm instance (shared across all agents)
        browser_use_llm = mainwin.browser_use_llm
        marketer = EC_Agent(mainwin=mainwin, skill_llm=llm, llm=browser_use_llm, task="", card=agent_card, skills=[worker_skill, chatter_skill], tasks=[worker_task, chatter_task])
