...
# DEPRECATED: My Twin Agent chatter skill - kept for reference, will be removed later
...
async def save_agent_skills(mainwin, skills):
    for skill in skills:
        logger.debug(f"tool_schemas: {len(tool_schemas)}.")
        if not skill_path:
    ...
    logger.debug(f"done built agent skills: {len(skills)} {[s.name for s in skills]}")
    return skills
