...
from agent.ec_skills.crawler.crawler_sessions import BrowserSession, ActionResult
from agent.ec_skills.browser.views import BrowserError
from agent.ec_skills.file_utils.file_system import FileSystem
from agent.ec_skills.browser.types import Page
import re
...
async def crawler_go_to_url(browser_session: BrowserSession, new_tab, url):
    try:
        if new_tab:
            ...
            page = await browser_session.create_new_tab(url)
            tab_idx = browser_session.tabs.index(page)
            memory = f'Opened new tab with URL {url}'
            msg = f'🔗  Opened new tab #{tab_idx} with url {url}'
            logger.info(msg)
        ...
        if any(
            ...
            site_unavailable_msg = f'Site unavailable: {url} - {error_msg}'
...
async def crawler_click_element_by_index(browser_session: BrowserSession, index):
    ...
    if index not in selector_map:
        ...
        logger.info(f'Element with index {index} not found in selector map, refreshing state...')
        ...
        if index not in selector_map:
            ...
            msg = f'Element with index {index} does not exist. Page has {len(selector_map)} interactive elements (indices 0-{max_index}). State has been refreshed - please use the updated element indices or scroll to see more elements'
    ...
    try:
        assert element_node is not None, f'Element with index {index} does not exist'
...
async def crawler_input_text(browser_session: BrowserSession, has_sensitive_data: bool = False, index, text):
    if index not in await browser_session.get_selector_map():
    ...
    try:
        ...
        msg = f'Failed to input text into element {index}.'
        raise BrowserError(msg)

    if not has_sensitive_data:
        msg = f'⌨️  Input {text} into index {index}'
    else:
        msg = f'⌨️  Input sensitive data into index {index}'
    logger.info(msg)
    logger.debug(f'Element xpath: {element_node.xpath}')
    return ActionResult(
...
async def crawler_switch_tab(browser_session: BrowserSession, page_id):
    await browser_session.switch_to_tab(page_id)
    page = await browser_session.get_current_page()
    try:
    ...
    # page was already loaded when we first navigated, this is additional to wait for onfocus/onblur animations/ajax to settle
        ...
        pass
    msg = f'🔄  Switched to tab #{page_id} with url {page.url}'
    logger.info(msg)
    return ActionResult(
        extracted_content=msg, include_in_memory=True, long_term_memory=f'Switched to tab {page_id}'
    )
...
async def crawler_close_tab(browser_session: BrowserSession, page_id):
    ...
    msg = f'❌  Closed tab #{page_id} with {url}, now focused on tab #{new_page_idx} with url {new_page.url}'
    logger.info(msg)
    return ActionResult(
        extracted_content=msg,
        include_in_memory=True,
        long_term_memory=f'Closed tab {page_id} with url {url}, now focused on tab {new_page_idx} with url {new_page.url}.',
    )
...
async def crawler_scroll(browser_session: BrowserSession, d):
    """
        ...
        try:
            ...
            if element_node is not None and params.index in selector_map:
                ...
                if result['success']:
                    ...
                    logger.debug(
                        f'Container scroll failed for element {params.index}: {result.get("reason", "Unknown")}')
...
async def crawler_read_cell_contents(cell_or_range: str, browser_session: BrowserSession):
    ...
    await crawler_select_cell_or_range(cell_or_range=cell_or_range, page=page)
...
async def crawler_fallback_input_into_single_selected_cell(text: str, page: Page):
    ...
    await page.keyboard.press(
        'Enter')  # make sure to commit the input so it doesn't get overwritten by the next action
    await page.keyboard.press('ArrowUp')
    return ActionResult(
