...
def subscribe_cloud_show(acctSiteID: str, id_token: str, ws_url: Optional[str] = None) -> Tuple[websocket.WebSocketApp, threading.Thread]:
    ...
    from agent.cloud_api.cloud_api import (
        _resolve_appsync_ws_url, _build_appsync_signed_url,
        _appsync_ws_reconnect_loop,
    )

    resolved_ws_url = _resolve_appsync_ws_url(ws_url, "CloudShow")
    ...
    logger.info("[CloudShow] Launching web socket thread with auto-reconnect")
    t = threading.Thread(
        target=_appsync_ws_reconnect_loop,
        args=("CloudShow", resolved_ws_url, id_token, build_ws),
        daemon=True,
    )
    t.start()
